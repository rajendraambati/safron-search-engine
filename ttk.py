...
def extract_data(xpath, driver, wait_time=15):
...
def scrape_google_maps(search_query, driver, max_companies=1000):
    try:
        ...
        try:
            ...
            logging.warning("No results found for this query")
            return None
        ...
        for idx, url in enumerate(listings):
            ...
            try:
                ...
                address = extract_data('//button[@data-item-id="address"]//div[@class="Io6YTe"]', driver)
...
def scrape_website_for_emails(url):
    """Enhanced email scraping with error handling"""
...
def main():
    ...
    # Fixed text input with proper label handling
    search_query = st.text_input(
        "Search Query",  # Visible label for accessibility
        key="search_input",
